...
val delta_path = "/mnt/deltalake/employees"
spark.read.format("csv")
.option("header", "true")
.option("inferSchema", "true")
.load("/FileStore/tables/employees.csv").write.format("delta").save(delta_path)
print("Created a Delta table at " + delta_path)

spark.read.format("delta").load(delta_path).createOrReplaceTempView("employees")


val renamedColumnsDF = specificColumnsDf.withColumnRenamed("emp_no", "number")
renamedColumnsDF.write
  .format("delta")
  .mode("overwrite")
  .save("/mnt/delta/employees")
  
  // merge schema to true ca sa updateze schema
renamedColumnsDF.write.format("delta") \
  .mode("append") \
...
# MAGIC %md
# MAGIC Audit Delta Lake Table History
# MAGIC All changes to the Delta table are recorded as commits in the table's transaction log. As you write into a Delta table or directory, every operation is automatically versioned. You can use the HISTORY command to view the table's history. For more information, check out the docs.

# COMMAND ----------

# MAGIC %fs ls /mnt/deltalake/employees/_delta_log/
...
# MAGIC Rollbacks
# MAGIC You can query by using either a timestamp or a version number using Python, Scala, and/or SQL syntax. For this example we will query a specific version using the Python syntax.
# MAGIC 
# MAGIC For more information, refer to Introducing Delta Time Travel for Large Scale Data Lakes and the docs.
# MAGIC 
...
# MAGIC 
# MAGIC %fs ls /sais_amer_20_demo/loans_delta/_delta_log/
# MAGIC %fs head /sais_amer_20_demo/loans_delta/_delta_log/00000000000000000014.json
...
# MAGIC %sh mkdir -p /dbfs/tmp/sais_amer_20_demo/loans/ && wget -O /dbfs/tmp/sais_amer_20_demo/loans/SAIS20-loan-risks.snappy.parquet  https://pages.databricks.com/rs/094-YMS-629/images/SAISEU19-loan-risks.snappy.parquet && ls -al  /dbfs/tmp/sais_amer_20_demo/loans/ 
...
parquet_path = "/sais_amer_20_demo/loans_parquet"
...
spark.read.format("parquet").load("/tmp/sais_amer_20_demo/loans") \
...
def random_checkpoint_dir(): 
  return "/sais_amer_20_demo/chkpt/%s" % str(random.randint(0, 10000))
...
def stop_all_streams():
  ...
  dbutils.fs.rm("/sais_amer_20_demo/chkpt/", True)
...
# MAGIC 
# MAGIC What really happened is that when the streaming query started adding new data to the Parquet table, it did not properly account for the existing data in the table. Furthermore, the new data files that were written out accidentally had two extra columns in the schema. Hence, when reading the table, the 2 different schemas from different files were merged together, thus unexpectedly modifying the schema of the table.
# MAGIC 
# MAGIC 
...
# MAGIC 
# MAGIC Parquet is only a data layout format within a single file, it does not provide any guarantees across an entire table of many parquet files.
# MAGIC 
# MAGIC #### 1. No schema enforcement 
# MAGIC Schema is not enforced when on writes, leading to dirty and often corrupted data.
# MAGIC 
...
delta_path = "/sais_amer_20_demo/loans_delta"
...
# MAGIC %md The writes were blocked because the schema of the new data did not match the schema of the Delta table (see the exception details). See more information about how it works [here](https://databricks.com/blog/2019/09/24/diving-into-delta-lake-schema-enforcement-evolution.html).
# MAGIC 
...
display(spark.read.format("delta").load(delta_path).sort(desc('closed')))
...
spark.read.format("delta") \
  .option("timestampAsOf", "2020-06-12T18:55:37.000+0000") \
  .load(delta_path) \
  .createOrReplaceTempView("loans_delta_pre_delete") \
...
# MAGIC 
# MAGIC While it's nice to be able to time travel to any previous version, sometimes you want to actually delete the data from storage completely for reducing storage costs or for compliance reasons (example, GDPR).
...
# Configure Delta Lake Silver Path
delta_small_path = "/sais_amer_20_demo/loans_delta_small"
...
loan_counts_by_states_path = "/sais_amer_20_demo/loans_by_states"
...
# loan_ids that haven't been completely paid off, random generated
